...
export type ControlSurfaceKind = 'panel' | 'activity';
...
export class ControlSurfaceRegistry implements vscode.Disposable {
    ...
    setActiveSidebarId(id: string | undefined): void {
        if (id && !this.items.some((item) => item.id === id && (item.kind === 'activity'))) {
            return;
        }
    ...
    add(view: ControlSurfaceViewInfo): void {
        ...
        if ((view.kind === 'activity') && !this.activeSidebarId) {
    ...
    removeById(id: string): ControlSurfaceViewInfo | undefined {
        ...
        if ((removed.kind === 'activity') && this.activeSidebarId === removed.id) {
            this.activeSidebarId = this.items.find((item) => item.kind === 'activity')?.id;
        }
        this.emitter.fire();
        return removed;
    }
